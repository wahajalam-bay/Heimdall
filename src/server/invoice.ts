...
export async function registerInvoice(user: SessionUser, input: InvoiceInput, db: DbClient = prisma) {
  ...
  const withholding =
    ...
    round2((subtotal * (await getConfigNumber(CONFIG_KEYS.WITHHOLDING_TAX_RATE, po.entityId, db))) / 100);
  const netPayable = round2(total - withholding);

  const number = await nextNumber(SEQ.INVOICE, db);
  ...
  const created = await db.invoiceItem.findMany({ where: { invoiceId: invoice.id } });
  for (const ci of created) {
    if (!ci.poItemId) continue;
    const poItem = await db.purchaseOrderItem.findUnique({ where: { id: ci.poItemId } });
    if (poItem) {
  ...
  await createTask(
    {
      ...
      priority: "NORMAL",
      slaHours: await getConfigNumber(CONFIG_KEYS.SLA_INVOICE_VERIFICATION_HOURS, po.entityId, db),
  ...
  // Run the match immediately so mismatches surface at registration.
  await verifyInvoice(user, invoice.id, db).catch(() => {
    /* verification failure is recorded on the invoice; registration stands */
  });
...
export async function verifyInvoice(user: SessionUser, invoiceId: string, db: DbClient = prisma) {
  ...
  const result = await runThreeWayMatch(invoiceId, db);
  const invoice = await db.invoice.findUnique({
    where: { id: invoiceId },
  ...
  if (!result.passed) {
    ...
    await completeTasks("INVOICE", invoice.id, user.id, db, "VERIFICATION");
  }
...
export async function decideInvoice(
  ...
  if (decision === "REJECTED") {
    await db.invoice.update({ where: { id: invoiceId }, data: { status: "REJECTED" } });
  } else if (decision === "RETURNED" || decision === "CLARIFICATION_REQUESTED") {
    await db.invoice.update({ where: { id: invoiceId }, data: { status: "UNDER_VERIFICATION" } });
...
export async function handoffToFinance(
  ...
  const number = await nextNumber(SEQ.HANDOFF, db);
  const handoff = await db.paymentHandoff.create({
    data: {
      number,
      invoiceId: invoice.id,
  ...
  await db.invoice.update({ where: { id: invoiceId }, data: { status: "SENT_TO_FINANCE" } });
  await completeTasks("INVOICE", invoiceId, user.id, db);

  if (invoice.po?.prId) {
    await transitionPr(user, invoice.po.prId, "FINANCE_HANDOFF", { force: true }, db);
  }
...
export async function recordPayment(
  ...
  await db.invoice.update({ where: { id: h.invoiceId }, data: { status: "PAID" } });
  await completeTasks("PAYMENT_HANDOFF", handoffId, user.id, db);
  ...
  if (h.invoice.poId) {
    ...
    const po = await db.purchaseOrder.findUnique({ where: { id: h.invoice.poId } });
    if (outstanding === 0 && po && po.status === "FULLY_RECEIVED") {
      const { closePo } = await import("./po");
      await closePo(user, po.id, "All goods received and all invoices paid", db).catch(() => {
        /* closure is best-effort here; the Open PO board will surface it otherwise */
      });
    }
  }
