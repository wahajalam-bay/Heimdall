...
export async function tagAssetsFromGrn(
  ...
  for (const li of grn.items) {
    ...
    for (let u = 0; u < units; u++) {
      const assetId = await nextNumber(SEQ.ASSET, db);
...
export async function createDisposalCase(
  ...
  const biddingThreshold = await getConfigNumber(CONFIG_KEYS.DISPOSAL_BIDDING_THRESHOLD, input.entityId, db);
  const number = await nextNumber(SEQ.DISPOSAL, db);

  const kase = await db.disposalCase.create({
    data: {
      number,
  ...
  for (const it of input.items) {
    ...
    const a = await db.asset.findUnique({ where: { id: it.assetId } });
    if (!a || ["DISPOSED", "SCRAPPED"].includes(a.status)) continue;
    await db.asset.update({ where: { id: it.assetId }, data: { status: "OBSOLETE" } });
    await db.assetTransaction.create({
      data: {
        assetId: it.assetId,
...
export async function advanceDisposal(
  ...
  if (to === "PENDING_APPROVAL") {
    const auditRequired = await getConfigBool(CONFIG_KEYS.DISPOSAL_REQUIRES_AUDIT, kase.entityId, db);
  ...
  if (to === "COMPLETED") {
    const mgmtThreshold = await getConfigNumber(CONFIG_KEYS.DISPOSAL_MGMT_APPROVAL_THRESHOLD, kase.entityId, db);
  ...
  if (to === "MANAGEMENT_APPROVAL" && input.winningBidId) {
    await db.disposalBid.updateMany({ where: { caseId }, data: { status: "LOST" } });
    await db.disposalBid.update({ where: { id: input.winningBidId }, data: { status: "WON" } });
    const bid = await db.disposalBid.findUnique({ where: { id: input.winningBidId } });
    if (bid) data.realisedValue = bid.amount;
  }

  const updated = await db.disposalCase.update({ where: { id: caseId }, data });
...
export async function addDisposalBid(
  ...
  const kase = await db.disposalCase.findUnique({ where: { id: input.caseId } });
  if (!kase) throw new NotFoundError("Disposal case");
  ...
  const bid = await db.disposalBid.create({
    data: {
      caseId: input.caseId,
  ...
  if (kase.stage === "APPROVED") {
    await db.disposalCase.update({ where: { id: input.caseId }, data: { stage: "BIDDING" } });
  }
  await writeAudit(
    {
      entityType: "DisposalCase",
