...
export function SectionCard({
  ...
  return (
    <section className={classNames("card overflow-hidden", className)}>
      {(title || actions) && (
        <header className="flex items-start justify-between gap-3 border-b border-[var(--c-border-subtle)] px-4 py-3">
          <div className="min-w-0">
            {title && <h3 className="text-[0.875rem] font-600 leading-5">{title}</h3>}
            {description && (
      ...
      <div className={classNames(bodyClassName ?? "px-4 py-3.5")}>{children}</div>
      {footer && (
        <footer className="border-t border-[var(--c-border-subtle)] bg-[var(--c-surface-sunken)] px-4 py-2.5">
          {footer}
        </footer>
      )}
    </section>
  ...
  return (
    <header className={classNames("flex flex-wrap items-start justify-between gap-x-6 gap-y-3", className)}>
      <div className="min-w-0 flex-1">
        ...
        {subtitle && (
          <p className="mt-1 max-w-3xl text-[0.8125rem] leading-5 text-[var(--c-text-secondary)]">{subtitle}</p>
        )}
...
export function StatTile({
  ...
  const body = (
    <div className="relative h-full overflow-hidden rounded-[var(--radius-lg)] border border-[var(--c-border)] bg-[var(--c-surface)] px-3.5 py-3 transition-colors hover:border-[var(--c-border-strong)]">
      <span
        aria-hidden
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {icon && <span className="text-[var(--c-text-tertiary)]">{icon}</span>}
      </div>
      <div className="mt-1.5 flex items-baseline gap-2">
        <span className="tnum text-[1.375rem] leading-7 font-600 tracking-[-0.02em]">{value}</span>
        {delta && (
          <span
      ...
      {hint && <div className="mt-1 text-xs text-[var(--c-text-secondary)]">{hint}</div>}
    </div>
  );

  return href ? (
    <Link href={href} className="block h-full focus-visible:rounded-[var(--radius-lg)]">
      {body}
    </Link>
  ) : (
    body
...
export function EmptyState({
  ...
  return (
    ...
    >
      <div className="mb-3 flex size-9 items-center justify-center rounded-[var(--radius-md)] border border-[var(--c-border)] bg-[var(--c-surface-sunken)] text-[var(--c-text-tertiary)]">
        {icon ?? (
      ...
      {description && (
        <p className="mt-1 max-w-sm text-xs leading-5 text-[var(--c-text-secondary)]">{description}</p>
      )}
...
export function ErrorState({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-6 py-12 text-center">
      <div className="mb-3 flex size-9 items-center justify-center rounded-[var(--radius-md)] border border-[var(--c-danger-border)] bg-[var(--c-danger-soft)] text-[var(--c-danger)]">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
      ...
      <h4 className="text-[0.8125rem] font-600">{title}</h4>
      {description && (
        <p className="mt-1 max-w-md text-xs leading-5 text-[var(--c-text-secondary)]">{description}</p>
      )}
      {action && <div className="mt-3.5">{action}</div>}
    </div>
  );
  ...
  return (
    <Card className="mx-auto max-w-lg">
      <div className="flex gap-3">
        <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-[var(--radius-md)] border border-[var(--c-warning-border)] bg-[var(--c-warning-soft)] text-[var(--c-warning)]">
        ...
        <div>
          ...
          <p className="mt-1 text-[0.8125rem] leading-5 text-[var(--c-text-secondary)]">
            {message ??
...
export function SkeletonTable({ rows = 8, cols = 6 }: { rows?: number; cols?: number }) {
  return (
    <div className="card overflow-hidden">
      <div className="flex gap-3 border-b border-[var(--c-border)] bg-[var(--c-surface-sunken)] px-3 py-2.5">
        {Array.from({ length: cols }).map((_, i) => (
          <div key={i} className="skeleton h-2.5 flex-1" />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, r) => (
        <div key={r} className="flex gap-3 border-b border-[var(--c-border-subtle)] px-3 py-3 last:border-0">
...
export function Meter({
  ...
  return (
    <div className="min-w-[5rem]">
      {(label || showValue) && (
        <div className="mb-1 flex items-baseline justify-between gap-2 text-2xs text-[var(--c-text-secondary)]">
...
export function Divider({ className }: { className?: string }) {
  return <hr className={classNames("border-0 border-t border-[var(--c-border-subtle)]", className)} />;
}
...
export function InlineAlert({
  ...
  const map = {
    info: ["var(--c-info-soft)", "var(--c-info-border)", "var(--c-info)"],
    success: ["var(--c-success-soft)", "var(--c-success-border)", "var(--c-success)"],
    warning: ["var(--c-warning-soft)", "var(--c-warning-border)", "var(--c-warning)"],
    danger: ["var(--c-danger-soft)", "var(--c-danger-border)", "var(--c-danger)"],
  }[tone];
  return (
    <div
      className="rounded-[var(--radius-sm)] border px-3 py-2 text-xs leading-5"
