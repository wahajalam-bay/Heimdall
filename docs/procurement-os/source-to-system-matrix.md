...
| RC-007 | BOTH | §4.7 | Where an inspection form is mandatory, store in-charge informs all concerned for physical inspection per **Annexure 4**; once done the form is **filled and signed by all concerns** | Zameen Media | Each named concern signs separately, with its own verdict and an attestation; the inspection cannot close while one is blank. **The Annexure 4 form itself is still outstanding** | `src/server/inspection-matrix.ts`, `src/server/receiving.ts` | — | — | **PARTIAL** | `verify-inspection-matrix.ts` | Annexure 4 form built to `image17.png` | P1 |
