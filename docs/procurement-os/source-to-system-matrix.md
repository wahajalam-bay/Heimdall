...
| **PR-003** Annexure 1 form fields | PARTIAL — 7 elements absent, 3 of them compulsory | **COMPLETE** — all 7 elements, and the form itself at `/pr/[id]/annexure-1`, printing every field from the requisition with sign/stamp/date/time from the approval attestation | `app/(app)/pr/[id]/annexure-1`, `server/attestation.ts` |
...
| **R-ZAM-004** Annexure A document set before finance | PARTIAL — all 30 document types optional | **IMPLEMENTED, gate off** — the seven documents seeded per entity and transaction type, the pack embedded on the invoice, and the requisition/order/receipt/invoice satisfied from the records themselves rather than re-uploaded. Enforcement is `invoice.enforce_payment_document_pack`, still off | `server/payment-pack.ts`, `components/domain/PaymentPackPanel.tsx` |
...
| ID | Src | § | Requirement | Entity | Current | Code | Tables | Permission | Status | Test | Required change | Pri |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R-002 | ZAM | §3.2 | Requesting departmental POCs define specifications, quality, brand, frequency of all demands; after delivery concerned dept heads or representatives verify specifications and sign the Material Inspection form | Zameen Media | The concerned department's POC is resolved at scheduling and held on the inspection; only they may sign Annexure 4's second block. An inspector signing it is refused by name | `src/server/receiving.ts`, `src/server/org.ts` | — | — | **COMPLETE** | `verify-annexure4.ts` | — | — |
...
| ID | Src | § | Requirement | Entity | Current | Code | Tables | Permission | Status | Test | Required change | Pri |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R-004 | ZAM | §3.4 | Procurement processes all invoices and ensures availability of supporting documents before submitting to finance **as per Annexure A** | Zameen Media | The Annexure A pack is seeded per entity and transaction type and embedded on the invoice. The requisition, order, receipt and invoice are satisfied by the records themselves, each linked to its printable form; the three conditional documents need an answer. Required / applicable / present / verified are four separate facts, and a waiver carries a reason and an approver | `src/server/payment-pack.ts`, `src/components/domain/PaymentPackPanel.tsx` | — | — | **COMPLETE, gate off** | `finance.test.ts` | Turn on `invoice.enforce_payment_document_pack` once the conditional answers are recorded for in-flight invoices | P1 |
...
| FI-003 | BOTH | §4.7 | A copy of the PO attached with relevant documents **per Annexure A**, then forwarded to Finance | Zameen Media | The order prints as its own document at `/po/[id]/document` — §4.6's authorised signatory, the vendor's acknowledgement, and the requisition it serves — and the pack counts it as held rather than asking for a scan of it | `src/app/(app)/po/[id]/document`, `src/server/payment-pack.ts` | — | — | **COMPLETE** | `documents.test.ts` | — | — |
...
| RC-005 | BOTH | §4.7 | Marketing collaterals and other items requiring departmental inspection are inspected accordingly | Zameen Media | Routed by the SOP chart: Printed Collateral → Store for all three checks; Furniture and Electronic Appliances → Admin; IT/Network/Mobiles → IT | `src/server/inspection-matrix.ts` | — | — | **COMPLETE** | `verify-inspection-matrix.ts` | — | — |
| RC-007 | BOTH | §4.7 | Where an inspection form is mandatory, store in-charge informs all concerned for physical inspection per **Annexure 4**; once done the form is **filled and signed by all concerns** | Zameen Media | Each named concern signs separately with its own verdict and attestation; the inspection cannot close while one is blank. Annexure 4 is built to `image17.png` with both signature blocks | `src/server/inspection-matrix.ts`, `src/server/receiving.ts`, `app/(app)/inspections/[id]/annexure-4` | — | — | **COMPLETE** | `verify-inspection-matrix.ts`, `verify-annexure4.ts` | — | — |
| RC-008 | BOTH | `image17` | Annexure 4 Goods/Material Inspection Note fields: **Receiving Date · Inspection Date · Supplier Name** · per line Sr.No, **Item Code**, Item Description, **Inspection Type (QUANTITATIVE: QTY, PASSED, REJECTED; QUALITATIVE/TECHNICAL: PASSED, REJECTED)**, **Expiry Date** · totals **Received Quantity, Inspected Quantity, Accepted Quantity, Returned Quantity** · certification text referencing **PO #** · **reason for rejection/return** · **Logistics (Received by): Name, Designation, Sign, Date** · **Concerned Department (Signature - POC): Name, Designation, Sign, Date** | Zameen Media | Built. The quantitative column prints a quantity plus pass and reject; the qualitative/technical column prints only pass or reject, which is the form's own distinction | `app/(app)/inspections/[id]/annexure-4/page.tsx`, `src/server/receiving.ts` | — | — | **COMPLETE** | `verify-annexure4.ts` | — | — |
| RC-009 | BOTH | Store Flow | Inspection responsibility matrix — **Technical / Qualitative / Quantitative** across Stationery, Giveaways, Furniture, Housekeeping & Grocery, IT/Network/Mobiles, Electronic Appliances, Printed Collateral; owners **Store / Admin / IT** | Zameen Media | All 21 cells transcribed from the chart and seeded as `InspectionResponsibility`. Each scheduled inspection snapshots the required sign-offs; the chart is silent on 11 of the 17 system categories and says so rather than guessing | `src/server/inspection-matrix.ts`, `scripts/seed-inspection-matrix.ts` | — | — | **COMPLETE** | `verify-inspection-matrix.ts` | Two chart columns (Giveaways, Electronic Appliances) have no system category yet | P3 |

## Inventory  ·  5 requirements

| ID | Src | § | Requirement | Entity | Current | Code | Tables | Permission | Status | Test | Required change | Pri |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| R-003 | ZAM | §3.3 | Manager Logistics maintains stock list with specifications and minimum stock level defined from past consumption or POC advice; responsible for receipt/issuance record keeping and must present it on demand | Zameen Media | Minimum stock records which of the two grounds it rests on; a suggested level is derived from issue history over a configurable window | `src/server/replenishment.ts` | — | — | **COMPLETE** | `verify-replenishment.ts` | — | — |
...
| ID | Src | § | Requirement | Entity | Current | Code | Tables | Permission | Status | Test | Required change | Pri |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| RC-020 | BOTH | Store Flow | Issuance: **(a) only against a PR form** · (b) **Issuance Slip signed by the receiver (user department)** · (c) Store Manager updates "Exit from Inventory" and checks balance against minimum stock level for recurring items · (d) **if minimum reached, Store Manager alerts the relevant procurement associate and a PR/PO is issued** | Zameen Media | (a) issue against requisition ✓ · (b) receiver acknowledgement, or a recorded paper slip, plus a printable slip ✓ · (c) ledger exit and minimum check ✓ · (d) alert fires on the issue that reaches the minimum, deferred after commit ✓. The requisition itself is raised by a person, deliberately | `src/server/stores.ts`, `src/server/replenishment.ts` | — | — | **COMPLETE** | `verify-issuance-slip.ts`, `verify-replenishment.ts` | — | — |
...
| PC-104 | BOTH | §4.4.1 + `image15` | Prescribed petty cash form filled with all required information, handed to requisitioner for **HOD approval**, then **approved by Director Procurement** | Zameen Media | Configurable approval, and the form itself at `/petty-cash/[id]/annexure-2` — both approvals as separate blocks, the quotation channel §4 asks for, and the PO-routing floor read from configuration rather than fixed in the page. The ZD Sr Mgr comparative step is unrepresented | `src/server/pettycash.ts`, `src/app/(app)/petty-cash/[id]/annexure-2` | — | — | **PARTIAL — form complete** | `finance.test.ts` | Per-entity route | P2 |
...
| PO-003 | BOTH | §4.6 | PO carries **the signature of Manager Procurement or other authorised signatory** | Zameen Media | The issuer is recorded as the authorised signatory on the order, with the time signed. Distribution channel, date and reference are held too — an order nobody can show was sent is one the vendor can deny receiving | `src/server/po.ts` | — | — | **COMPLETE** | `verify-po-acknowledgement.ts` | — | — |
...
| RC-010 | BOTH | Store Flow | If goods pass inspection, Store Manager proceeds to Goods Received. **If inspection fails, a Return-to-Vendor (RTV) document is lodged by the relevant inspector within the ERP** | Zameen Media | The RTV comes off the inspection: failed quantities from the inspection, prices from the order, nothing retyped. The inspector may lodge it, under a re-verified cascade rather than a bypass | `src/server/receiving-exceptions.ts` | — | — | **COMPLETE** | `verify-rtv.ts` | — | — |
...
| SO-004 | BOTH | §4.5.1 | **Preparation of Comparative Statement, Negotiation Minutes, approval documents and conclusion is to be documented** | Zameen Media | The Comparative Statement exists. Negotiation Minutes are now their own signed document: participants on both sides as rows, each of the six named bases answered, a required conclusion, and a hash of what was signed | `src/server/negotiation-minutes.ts` | — | — | **COMPLETE** | `verify-negotiation-minutes.ts` | — | — |
...
| V-019 | ZAM | §5.2 + `image20` | Vendor Selection Form: Prepared By, Designation, Date, Vendor Referred By; Vendor Information incl. company name/address/city/contact/type of business/representative/vendor type **and "Any other company owned by same owner"**; **Min. Qualifying Score 30/60**; sections Tax status (10), Company History (10), Key Client Reference Check (12), Payment Mode (10), Company Registration (5), Company Setup (10), Internal Reference (4); Mandatory documents **FBR Online Status, Company Registration, Job Completion Certificate from Clients**; signatures **Prepared By / Verified By / Approved By** | Zameen Media | The form is built at `/vendors/[id]/annexure-6`: preparer block, vendor information, the related-party question, the three mandatory documents (two of which needed their own `docType` — filer status is not the NTN certificate, a client's completion certificate is not a reference letter), and three signature blocks. **The instrument behind it is still not the form's**: all 20 criteria sit in six groups Annexure 6 does not name, so every one reports off-form rather than being force-fitted, and the sheet says so once at the top. The form's seven maxima sum to 61 against a stated 30/60 — printed as given, unresolved | `src/server/vendor-selection-form.ts`, `src/app/(app)/vendors/[id]/annexure-6` | — | — | **PARTIAL — form complete, instrument not** | `vendors.test.ts` | Rebuild the PQ instrument to the seven sections, once the 60-versus-61 question is answered. The Verified By signature has no third act to bind to and prints blank | P0 |
...
| R-ZAM-002 | §3.2 | Requesting departmental POCs are responsible to define specifications, quality, brand, frequency of all required demands. **After delivery the concerned departmental head or their representative verifies the specifications and signs the Material Inspection form** | Requesting department POC / HOD | Annexure 4's second signature block is routed to the requisition's department POC, captured at scheduling so a later reorganisation cannot change who was supposed to sign. Where no POC is appointed the department head may stand in, and that substitution is visible | `src/server/receiving.ts` | **COMPLETE** | — | — |
| R-ZAM-003 | §3.3 | Manager Logistics is responsible to maintain the stock list with specifications and **minimum stock level defined on the basis of past history (consumption) or on advice of concerned departmental POCs**, for record keeping of receipt and issuance, and is liable to present it as and when required | Manager Logistics | A minimum carries its basis — consumption, POC advice, or hand-set — plus who set it and when. POC advice without attribution is refused. A suggestion is derived from the ledger, and withheld where there are too few issues to call it a pattern | `src/server/replenishment.ts` | **COMPLETE** | — | — |
